...
srun -p $partition -w SH-IDCA1404-10-140-54-54 --job-name=${JOB_NAME} ${SRUN_ARGS} \
    --mpi=pmi2 --quotatype=${TYPE} \
    --cpus-per-task=$CPUS_PER_TASK \
    --kill-on-bad-exit=1 \
    python -u carla_train_eval.py --device=5 --large --fusion='feature' --log_name='feature_5frame_nnres_sepenc' --n_frame=5 --res --nn_res --separate_encoder --large 2>&1 | tee run_logs/feature_5frame_nnres_sepenc_large.log