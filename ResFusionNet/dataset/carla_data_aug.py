...
def dbsample(CLASSES, data_root, data_dict, db_sampler, sample_groups, dim):
    '''
    ...
    for name, v in sample_groups.items():
        ...
        for i in range(n_gt, len(coll_mat)):
            if any(coll_mat[i]):
                ...
                sampled_pts_cur = read_points(pt_path, dim=dim)
...
def carla_point_range_filter(data_dict, point_range):
    '''
    data_dict: dict(pts, gt_bboxes_3d, gt_labels, gt_names, difficulty)
...
def carla_lidar_radar_point_range_filter(data_dict, point_range):
    '''
    data_dict: dict(pts, gt_bboxes_3d, gt_labels, gt_names, difficulty)
    point_range: [x1, y1, z1, x2, y2, z2]
    '''
    lidar_pts = data_dict['lidar_pts']
    radar_pts = data_dict['radar_pts']
    lidar_flag_x_low = lidar_pts[:, 0] > point_range[0]
    lidar_flag_y_low = lidar_pts[:, 1] > point_range[1]
    lidar_flag_z_low = lidar_pts[:, 2] > point_range[2]
    lidar_flag_x_high = lidar_pts[:, 0] < point_range[3]
    lidar_flag_y_high = lidar_pts[:, 1] < point_range[4]
    lidar_flag_z_high = lidar_pts[:, 2] < point_range[5]
    
    radar_flag_x_low = radar_pts[:, 0] > point_range[0]
    radar_flag_y_low = radar_pts[:, 1] > point_range[1]
    radar_flag_z_low = radar_pts[:, 2] > point_range[2]
    radar_flag_x_high = radar_pts[:, 0] < point_range[3]
    radar_flag_y_high = radar_pts[:, 1] < point_range[4]
    radar_flag_z_high = radar_pts[:, 2] < point_range[5]
    
    lidar_keep_mask = lidar_flag_x_low & lidar_flag_y_low & lidar_flag_z_low & lidar_flag_x_high & lidar_flag_y_high & lidar_flag_z_high
    radar_keep_mask = radar_flag_x_low & radar_flag_y_low & radar_flag_z_low & radar_flag_x_high & radar_flag_y_high & radar_flag_z_high
    
    lidar_pts = lidar_pts[lidar_keep_mask]
    radar_pts = radar_pts[radar_keep_mask]
    
    data_dict.update({'lidar_pts': lidar_pts})
    data_dict.update({'radar_pts': radar_pts})
    
    return data_dict


def single_object_filter(gt_bboxes_3d, object_range):
    # print(f'gt_bboxes_3d: {type(gt_bboxes_3d)}\n{gt_bboxes_3d}')
    # print(f'object_range: {type(object_range)}\n{object_range}')
...
def carla_data_augment(CLASSES, data_root, data_dict, data_aug_config, dim=4):
    '''
    CLASSES: dict(Pedestrian=0, Cyclist=1, Car=2)
    data_root: str, data root
    data_dict: dict(pts, gt_bboxes_3d, gt_labels, gt_names, difficulty)
    ...
    print(f'bbox 111: {data_dict["gt_bboxes_3d"].shape}')
    ...
    print(f'bbox 222: {data_dict["gt_bboxes_3d"].shape}')
    ...
    data_dict = carla_point_range_filter(data_dict, point_range)

    print(f'bbox 333: {data_dict["gt_bboxes_3d"].shape}')
    # 6. object range filter
    object_range = data_aug_config['object_range_filter']
    data_dict = object_range_filter(data_dict, object_range)
    print(f'bbox 444: {data_dict["gt_bboxes_3d"].shape}')
