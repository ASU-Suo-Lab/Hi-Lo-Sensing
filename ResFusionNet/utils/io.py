...
def save_dict_to_npz(filename, data_dict):
    """
    Save a dictionary containing numpy arrays and other data types to a .npz file.
...
def read_json(file_path, suffix='.json'):
    assert os.path.splitext(file_path)[1] == suffix, f"File must have {suffix} extension"
...
def read_points(file_path, dim=4):
    ...
    if suffix == '.bin':
        return np.fromfile(file_path, dtype=np.float64).reshape(-1, dim)
    else:
