...
from utils import setup_seed, carla_keep_bbox_from_lidar_range
from dataset import Carla, carla_get_dataloader
from model import CarlaPointPillars as PointPillars
...
def main(args):
    setup_seed()
    train_dataset = Carla(data_root=args.data_root,
                          split='train')
    val_dataset = Carla(data_root=args.data_root,
                        split='val')
    CLASSES = Carla.CLASSES
    LABEL2CLASSES = {v:k for k, v in CLASSES.items()}
    train_dataloader = carla_get_dataloader(dataset=train_dataset, 
                                      batch_size=args.batch_size, 
                                      num_workers=args.num_workers,
                                      shuffle=True)
    val_dataloader = carla_get_dataloader(dataset=val_dataset, 
                                        batch_size=args.batch_size, 
                                        num_workers=args.num_workers,
                                        shuffle=False)
    ...
    for epoch in range(args.max_epoch):
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            ...
            batched_pts = [data[:, :4] for data in data_dict['batched_pts']]
            batched_gt_bboxes = data_dict['batched_gt_bboxes']
            batched_labels = data_dict['batched_labels']
            batched_difficulty = data_dict['batched_difficulty']
            
            # print(f'batched_pts: {len(batched_pts)} | {batched_pts[0].shape}\n{batched_pts[0]}')
            # print(f'batched_gt_bboxes: {len(batched_gt_bboxes)} | {batched_gt_bboxes[0].shape}\n{batched_gt_bboxes[0]}')
            # print(f'batched_labels: {len(batched_labels)} | {batched_labels[0].shape}\n{batched_labels[0]}')
            
            # for gt_bboxes in batched_gt_bboxes:
            #     if gt_bboxes.shape[0] == 0:
            #         print(batched_gt_bboxes)    
            #         assert False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', default='/mnt/disk1/yihong/PointPillars/data/ff', 
