...
class PillarLayer(nn.Module):
    ...
    def forward(self, batched_pts):
        ...
        for i, pts in enumerate(batched_pts):
            ...
            # print(f'pts: {pts.shape}, voxels_out: {voxels_out.shape}, coors_out: {coors_out.shape}, num_points_per_voxel_out: {num_points_per_voxel_out.shape}')
        
        pillars = torch.cat(pillars, dim=0) # (p1 + p2 + ... + pb, num_points, c)
        npoints_per_pillar = torch.cat(npoints_per_pillar, dim=0) # (p1 + p2 + ... + pb, )
        coors_batch = []
        for i, cur_coors in enumerate(coors):
            # print(f'cur_coors: {cur_coors}')
            coors_batch.append(F.pad(cur_coors, (1, 0), value=i))
            # print(f'coors_batch[-1]: {coors_batch[-1]}')
        coors_batch = torch.cat(coors_batch, dim=0) # (p1 + p2 + ... + pb, 1 + 3)

        # print(f'coors_batch: {coors_batch}')
        
        # print(f'pillars shape: {pillars.shape}, coors_batch shape: {coors_batch.shape}, npoints_per_pillar shape: {npoints_per_pillar.shape}')
...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # print(f'offset_pt_center: {offset_pt_center.shape}, x_offset_pi_center: {x_offset_pi_center.shape}, y_offset_pi_center: {y_offset_pi_center.shape}')
        ...
        # print(f'pooling_features: {pooling_features.shape}')
        ...
        print(f'batched_canvas: {batched_canvas.shape}')
        # assert False
...
class CarlaPointPillars(nn.Module):
    def __init__(self,
                 nclasses=3, 
                 voxel_size=[0.16, 0.16, 4],
                 point_cloud_range=[-37, -50, -0.1, 40, 29, 3.9],
                 max_num_points=32,
        ...
        self.pillar_encoder = PillarEncoder(voxel_size=voxel_size, 
                                            ...
                                            in_channel=9, 
                                            out_channel=64,
                                            x_l=168, y_l=152) # after this we got pseude-image with shape (batch_size, out_channel, y_l, x_l)
    ...
    def forward(self, batched_pts, batched_radar_pts=None, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        batch_size = len(batched_pts)
        # batched_pts: list[tensor] -> pillars: (p1 + p2 + ... + pb, num_points, c), 
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3), 
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size)
        pillars, coors_batch, npoints_per_pillar = self.pillar_layer(batched_pts)
        # print(f'pillar shape: {pillars.shape}, coors_batch shape: {coors_batch.shape}, npoints_per_pillar shape: {npoints_per_pillar.shape}')
        ...
        #     radar_pillars, radar_coors_batch, radar_npoints_per_pillar = self.pillar_layer(batched_radar_pts)
        #     radar_pillar_features = self.pillar_encoder(radar_pillars, radar_coors_batch, radar_npoints_per_pillar)
        # print(f'pillar_features: {pillar_features.shape}')
        # print(f'radar_pillar_features: {radar_pillar_features.shape}')
        
        # # assert False

        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs = self.backbone(pillar_features) # a list
        ...
        # print(f'bbox_cls_pred: {bbox_cls_pred.shape}, bbox_pred: {bbox_pred.shape}, bbox_dir_cls_pred: {bbox_dir_cls_pred.shape}')
        ...
        if mode == 'train':
            ...
            print(f'bbox_cls_pred: {bbox_cls_pred.shape}, bbox_pred: {bbox_pred.shape}, bbox_dir_cls_pred: {bbox_dir_cls_pred.shape}, batched_anchors: {len(batched_anchors)}')
