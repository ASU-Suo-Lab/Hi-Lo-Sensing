...
if [ "$#" -lt 3 ]; then
    echo "Usage: bash scripts/carla_dsvt_eval_low_thresh.sh <gpus> <mha_legacy|sdpa|flash_attn> <ckpt> [extra_tag] [eval_tag] [additional test.py args...]"
    exit 1
fi
...
case "${ATTN_BACKEND}" in
    mha_legacy)
        ...
        DEFAULT_EVAL_TAG="carla_mha_legacy_low_thresh_eval"
        ;;
    sdpa)
        DEFAULT_EXTRA_TAG="carla_sdpa_train"
        DEFAULT_EVAL_TAG="carla_sdpa_low_thresh_eval"
        ;;
    flash_attn)
        DEFAULT_EXTRA_TAG="carla_flash_attn_fastest_train"
        DEFAULT_EVAL_TAG="carla_flash_attn_fastest_low_thresh_eval"
        ;;
    *)
...
COMMON_ARGS=(
    ...
    --eval_tag "${EVAL_TAG}"
    --set
    MODEL.BACKBONE_3D.ATTN_BACKEND "${ATTN_BACKEND}"
    MODEL.POST_PROCESSING.SCORE_THRESH 0.0
    MODEL.DENSE_HEAD.POST_PROCESSING.SCORE_THRESH 0.0
)
