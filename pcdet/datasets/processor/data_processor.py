...
class DataProcessor(object):
    def __init__(self, processor_configs, point_cloud_range, training, num_point_features):
        ...
        self.radar_voxel_generator = None
    ...
    def transform_radar_points_to_voxels(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.transform_radar_points_to_voxels, config=config)

        if self.radar_voxel_generator is None:
            self.radar_voxel_generator = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,
                coors_range_xyz=self.point_cloud_range,
                num_point_features=config.NUM_RADAR_POINT_FEATURES,
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL,
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode],
            )

        radar_points = data_dict['radar_points']
        voxel_output = self.radar_voxel_generator.generate(radar_points)
        radar_voxels, radar_coordinates, num_radar_points = voxel_output
        ...
        data_dict['radar_voxel_num_points'] = num_radar_points
        return data_dict
