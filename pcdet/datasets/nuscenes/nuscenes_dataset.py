...
class NuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.radar_config is not None:
            ...
            self.radar_sweeps_num = self.radar_config.SWEEPS_NUM
        else:
    ...
    def get_radar_with_sweeps(self,pts_filename):
        """
    ...
    def load_radar_info(self, input_dict, info):
        ...
        for key, sweeps in radars_dict.items():
            if len(sweeps) < self.radar_sweeps_num:
                idxes = list(range(len(sweeps)))
            else:
                idxes = list(range(self.radar_sweeps_num))

            ts = sweeps[0]['timestamp'] * 1e-6
            for idx in idxes:
                sweep = sweeps[idx]

                points_sweep = self.get_radar_with_sweeps(sweep['data_path'])
                points_sweep = np.copy(points_sweep).reshape(-1, self.radar_load_dim)
