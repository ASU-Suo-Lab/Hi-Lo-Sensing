...
def random_flip_along_x(gt_boxes, points, radar_points, return_flip=False, enable=None):
    """
    Args:
...
def random_flip_along_y(gt_boxes, points, radar_points, return_flip=False, enable=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, radar_points, rot_range, return_rot=False, noise_rotation=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    if radar_points is not None:
        radar_points = common_utils.rotate_radar_points_along_z(radar_points[np.newaxis, :, :], np.array([noise_rotation]))[0]
...
def global_scaling(gt_boxes, points, radar_points, scale_range, return_scale=False):
    """
    Args:
