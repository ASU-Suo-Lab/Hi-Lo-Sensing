...
class DataAugmentor(object):
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        radar_points = data_dict['radar_points'] if 'radar_points' in data_dict else None
            
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            gt_boxes, points, radar_points, enable = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
            ...
            if 'roi_boxes' in data_dict.keys():
                ...
                roi_boxes, _, _, _ = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
                    data_dict['roi_boxes'].reshape(-1,dim), np.zeros([1,3]), np.zeros([1,3]), return_flip=True, enable=enable
                )
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        gt_boxes, points, radar_points, noise_rot = augmentor_utils.global_rotation(
            data_dict['gt_boxes'], data_dict['points'], radar_points, rot_range=rot_range, return_rot=True
        )
        if 'roi_boxes' in data_dict.keys():
            num_frame, num_rois,dim = data_dict['roi_boxes'].shape
            roi_boxes, _, _, _ = augmentor_utils.global_rotation(
                data_dict['roi_boxes'].reshape(-1, dim), np.zeros([1, 3]), np.zeros([1, 3]), rot_range=rot_range, return_rot=True, noise_rotation=noise_rot)
            data_dict['roi_boxes'] = roi_boxes.reshape(num_frame, num_rois,dim)
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        if 'roi_boxes' in data_dict.keys():
            ...
            gt_boxes, points, radar_points, noise_scale = augmentor_utils.global_scaling(
                data_dict['gt_boxes'], data_dict['points'], radar_points, config['WORLD_SCALE_RANGE'], return_scale=True
            )
    ...
    def random_world_translation(self, data_dict=None, config=None):
        ...
        if 'radar_points' in data_dict:
            data_dict['radar_points'][:, :3] += noise_translate
        return data_dict
