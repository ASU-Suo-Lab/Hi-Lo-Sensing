...
class LidarRadar2(Detector3DTemplate):
    ...
    def build_ms_atten(self, model_info_dict):
        if self.model_cfg.get('MS_ATTEN', None) is None:
            return None, model_info_dict
    
        ms_atten_module = fuser.__all__[self.model_cfg.MS_ATTEN.NAME](
            model_cfg=self.model_cfg.MS_ATTEN
        )
        model_info_dict['module_list'].append(ms_atten_module)
        return ms_atten_module, model_info_dict
