...
class LRMSDeformAttnFuser1(LRMSDeformAttnFuser):
    def __init__(self, model_cfg) -> None:
        super().__init__(model_cfg=model_cfg)
        self.DeformAttn1 = MSDeformAttn(d_model=self.radar_post_dim+self.lidar_post_dim,d_v=self.lidar_post_dim, n_levels=1, n_heads=8, n_points=8)  # d_model=256, n_levels=1, n_heads=8, n_points=4
        self.DeformAttn2 = MSDeformAttn(d_model=self.lidar_post_dim+self.radar_post_dim,d_v=self.radar_post_dim, n_levels=1, n_heads=8, n_points=8)  # d_model=256, n_levels=1, n_heads=8, n_points=4
        self.LearnedPositionalEncoding = LearnedPositionalEncoding3D(num_feats=(self.radar_post_dim+self.lidar_post_dim) // 2,
    ...
    def forward(self, batch_dict):
        ...
        pos1 = self.LearnedPositionalEncoding1(mask)
        pos2 = self.LearnedPositionalEncoding2(mask)
        pos = self.LearnedPositionalEncoding(mask)

        reference_point1 = self.get_reference_points(self.bev_size, self.bev_size, device=device)
        reference_point2 = self.get_reference_points(self.bev_size, self.bev_size, device=device)

        radar_lidar_feats = torch.cat((radar_feats, lidar_feats), dim=-1)

        fusion_f1 = self.DeformAttn1(query=self.with_pos_embed(radar_lidar_feats, pos),
                                         reference_points=reference_point1,
                                         ...
                                         input_level_start_index=torch.tensor(
                                                 [0, self.bev_size * self.bev_size]).to(device),
                                         input_padding_mask=None)
        fusion_f2 = self.DeformAttn2(query=self.with_pos_embed(radar_lidar_feats, pos),
                                         reference_points=reference_point2,
